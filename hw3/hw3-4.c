...
void min_heapify(struct container *C, int now_offset, int size, int n){
  int offset = (n+1)/2;
  int left, right, now_tmp;
  struct container temp;

  int now = now_offset - offset;
  ...
  while (now < size) {
    ...
    now_tmp = now;
    if (left < size && compare_container_arr(C, now_tmp + offset, left + offset) > 0)
      now_tmp = left;

    if (right < size && compare_container_arr(C, now_tmp + offset, right + offset) > 0)
      now_tmp = right;
    
    if (now_tmp == now)
      break;

    swap_container_arr(C, now + offset, now_tmp + offset, &temp);
    now = now_tmp;    
  }
}
...
void build_max_heap_left(struct container *C, int n){
  int left_n = (n+1)/2;
  for (int i = (left_n-1)/2; i >= 0; i--){
    max_heapify(C, i, left_n);
  }
}

void build_min_heap_right(struct container *C, int n){
  int right_n = n/2;
  for (int i = (n-1) - right_n/2; i >= n-right_n; i--){
    min_heapify(C, i, right_n, n);
  }
}

void swap_root(struct container *C, int n){
  struct container temp;
  if (compare_container_arr(C, 0, (n+1)/2) > 0){
    swap_container_arr(C, 0, (n+1)/2, &temp);
  }
}

int is_same(struct container *M3, struct container *C, int n){
  if (compare_container(M3+1, C) == 0){
    if (compare_container(M3+2, C+(n+1)/2) == 0){
      if (compare_container_arr(C, 1, 2) >= 0){
        if (compare_container(M3, C+1) == 0){
            return 1;
          }
        }
      else {
        if (compare_container(M3, C+2) == 0){
          return 1;
        }
      }
    }
    ...
    struct container *M3, struct container *C, int n)
{
  ...
  while (1){
    ...
    copy_container(M3+1, C);
    if (compare_container_arr(C, 1, 2) >= 0){
      copy_container(M3, C+1);
    }
    else {
      copy_container(M3, C+2);
    }
    copy_container(M3+2, C+(n+1)/2);

    swap_root(C, n);

    max_heapify(C, 0, (n+1)/2);
    min_heapify(C, (n+1)/2, n/2, n);

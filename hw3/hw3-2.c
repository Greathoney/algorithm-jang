...
void quick_sort(struct container *M3, struct container *C, int low, int high, int mid){
  struct container temp;

  if (high - low == 1){
    if (compare_container_arr(C, low, high) > 0){
      swap_container_arr(C, low, high, &temp);
    }
    return;
  }

  int pivot = low;
  int now_low = low + 1, now_high = high;

  while (now_high - now_low != -1){
    if (compare_container_arr(C, now_low, pivot) < 0){
        now_low++;
        continue;
    }
    if (compare_container_arr(C, pivot, now_high) < 0){
        now_high--;
        continue;
    }
    swap_container_arr(C, now_low, now_high, &temp);
  }

  swap_container_arr(C, pivot, now_high, &temp);

  if (mid <= now_high && now_high <= mid+2){
    copy_container(M3+now_high-mid, C+now_high);
  }

  if (low < now_high - 1 && now_high-1 >= mid){
    quick_sort(M3, C, low, now_high-1, mid);
  }
  if (now_low < high && now_low <= mid + 2){
    quick_sort(M3, C, now_low, high, mid);
  }
}
...
{
  /* FILL */
  quick_sort(M3, C, 0, n-1, (n+1)/2-2);
  for (int i = 0; i < 3; i++){
    copy_container(M3+i, C+(n+1)/2-2+i);
  }
}
